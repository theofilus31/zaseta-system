...
const avatarStatusVariants = cva('flex items-center rounded-full size-2 border-2 border-sc-background', {
  variants: {
    variant: {
      online: 'bg-green-600',
...
function AvatarFallback({ className, ...props }: React.ComponentProps<typeof AvatarPrimitive.Fallback>) {
  return (
    <AvatarPrimitive.Fallback
      ...
      className={cn(
        'flex h-full w-full items-center justify-center rounded-full border border-sc-border bg-sc-accent text-sc-accent-foreground text-xs',
        className,
      )}
      {...props}
    />
