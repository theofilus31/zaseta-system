...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        'border-sc-border/50 bg-sc-background grid min-w-[8rem] items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs shadow-xl',
        className,
      )}
    >
        ...
        {payload.map((item, index) => {
          ...
          return (
            <div
              key={item.dataKey}
              className={cn(
                '[&>svg]:text-sc-muted-foreground flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5',
              ...
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  ...
                  >
                    <div className="grid gap-1.5">
                      {nestLabel ? tooltipLabel : null}
                      <span className="text-sc-muted-foreground">{itemConfig?.label || item.name}</span>
                    </div>
                    {item.value && (
                      <span className="text-sc-foreground font-mono font-medium tabular-nums">
    ...
    <div className={cn('flex items-center justify-center gap-4', verticalAlign === 'top' ? 'pb-3' : 'pt-3', className)}>
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className={cn('[&>svg]:text-sc-muted-foreground flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3')}
          >
