...
/**
 * Nama berkas ini sengaja `sc-card.tsx`, BUKAN `card.tsx` seperti biasanya
 * shadcn CLI menamainya. `Card.jsx` (huruf besar) sudah ada di folder yang
 * sama -- di Windows (filesystem case-INSENSITIVE) itu bikin bundler
 * salah resolve `@/components/ui/card` ke `Card.jsx` lama, bukan ke
...
// Variants
const cardVariants = cva('flex flex-col items-stretch text-sc-card-foreground rounded-xl', {
  variants: {
    variant: {
      default: 'bg-sc-card border border-sc-border shadow-xs black/5',
      accent: 'bg-sc-muted shadow-xs p-1',
    },
  },
  defaultVariants: {
    variant: 'default',
...
const cardHeaderVariants = cva('flex items-center justify-between flex-wrap px-5 min-h-14 gap-2.5', {
  variants: {
    variant: {
      default: 'border-b border-sc-border',
      accent: '',
    },
  },
  defaultVariants: {
...
const cardContentVariants = cva('grow p-5', {
  variants: {
    variant: {
      default: '',
      accent: 'bg-sc-card rounded-t-xl [&:last-child]:rounded-b-xl',
    },
  },
  defaultVariants: {
    variant: 'default',
...
const cardFooterVariants = cva('flex items-center px-5 min-h-14', {
  variants: {
    variant: {
      default: 'border-t border-sc-border',
      accent: 'bg-sc-card rounded-b-xl mt-[2px]',
    },
  },
  defaultVariants: {
    variant: 'default',
...
function CardDescription({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div data-slot="card-description" className={cn('text-sm text-sc-muted-foreground', className)} {...props} />;
}
