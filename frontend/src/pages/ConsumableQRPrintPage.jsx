...
/**
 * Cetak label barcode barang habis pakai — padanan QRPrintPage.jsx (aset).
 * QrLabelDesign dipakai APA ADANYA (tidak diubah) -- komponennya sudah
 * generik, cuma butuh { name, asset_code, image_path }, jadi kode barang
 * (`qr.item_code`) dipetakan ke `asset_code` saat memanggilnya di bawah.
 */
const SIZE_OPTIONS = [2, 3, 4, 5].map((s) => ({ value: s, label: `${s}×${s} cm` }));

export default function ConsumableQRPrintPage() {
  const { id } = useParams();
  const [qr, setQr] = useState(null);
  const [sizeCm, setSizeCm] = useState(3);

  useEffect(() => {
    axiosClient.get(`/consumables/${id}/qr/print`).then((res) => setQr(res.data));
  }, [id]);
  ...
  const labelData = { name: qr.name, asset_code: qr.item_code, image_path: qr.image_path };
  ...
  return (
    <>
      ...
      <div className="print-hide">
        <PageHeader
          backTo={`/consumables/${id}`}
          backLabel="Detail Barang"
          eyebrow="Cetak Label"
          title={qr.name}
          description={<span className="font-mono text-ink-600">{qr.item_code}</span>}
          actions={
      ...
      <Card padded={false} className="overflow-hidden">
        <div className="print-hide">
          <CardHeader
            ...
            bordered
            action={<SegmentedControl options={SIZE_OPTIONS} value={sizeCm} onChange={setSizeCm} />}
          />
        </div>
        ...
        <div className="flex justify-center bg-ink-50 py-10 print:bg-white print:py-0">
          ...
          >
            <QrLabelDesign asset={labelData} sizeCm={sizeCm} />
          </div>
        </div>
      </Card>
